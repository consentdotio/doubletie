...
describe('integration: slug mixin', () => {
	...
	it('should generate a slug during insertion', async () => {
		...
		const mockPost = { id: 1, title: 'Test Integration Post', slug: 'test-integration-post' };
	...
	it('should not create duplicate when using insertIfNotExistsWithSlug', async () => {
		...
		const existingPost = { id: 1, title: 'Already Exists', slug: 'already-exists' };
		
		// Mock insertWithSlug and insertIfNotExistsWithSlug
		PostModel.insertWithSlug = vi.fn().mockResolvedValue(existingPost);
		PostModel.insertIfNotExistsWithSlug = vi.fn().mockResolvedValue(existingPost);
	...
	it('should upsert with slug generation', async () => {
		// Mock upsert results
		const initialPost = { id: 1, title: 'Initial Title', slug: 'initial-title' };
		const updatedPost = { id: 1, title: 'Updated Title', slug: 'updated-title' };
	...
	it('should handle batch inserts with slug generation', async () => {
		...
		await db.insertInto('posts').values(processedData as any).execute();
